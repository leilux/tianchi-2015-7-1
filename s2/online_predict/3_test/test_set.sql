...
create table online_test_set
    ...
    from
		...
		online_test_feature_ui_behavior_count a          -- 1

		left outer join
		online_test_feature_ui_behavior_lasttime_delta b -- 2
		on a.user_id=b.user_id and a.item_id=b.item_id

		left outer join
		online_test_feature_u_behavior_count c           -- 3  
		on a.user_id=c.user_id and a.item_id=c.item_id

		left outer join
		online_test_feature_i_sale_count d               -- 4
		on a.user_id=d.user_id and a.item_id=d.item_id

		left outer join
		online_test_feature_ui_base e                    -- 5
		on a.user_id=e.user_id and a.item_id=e.item_id


		left outer join
		online_test_feature_u_base f                     -- 6
		on a.user_id=f.user_id and a.item_id=f.item_id

		left outer join
		online_test_feature_i_click_count g              -- 7
		on a.user_id=g.user_id and a.item_id=g.item_id

		left outer join
		online_test_feature_i_star_count h               -- 8
		on a.user_id=h.user_id and a.item_id=h.item_id

		left outer join
		online_test_feature_i_cart_count i               -- 9
